var model     = require('./model'),
    ...
    schema    = {
      ...
      // The name in PARTS, when a source gave them to us separately. Both LTI
      // versions do (1.3 `given_name`/`family_name`, 1.1
      // `lis_person_name_given`/`_family`) and we used to keep only the joined
      // string, leaving every surname-ordered view to GUESS at the surname —
      // which cannot be right for names whose family name comes first.
      // Optional: absent for users who predate this, who fall back to
      // derivation (public/js/courseEditor/rosterSort.js), so no backfill is
      // needed and ordering becomes exact over time.
      ...
      avatar      : { type: String },
      source      : { type: String,  default: 'firebase' },
      profiles    : {},
      settings       : {
...
// Bcrypt middleware (local email/password auth). No-op for firebase/google/lti
// accounts, which never set a password.
...
// Password verification (local auth)
...
function normalizeAvatar() {
  ...
  var hasCloudConfig = cloudHost.startsWith('https://') && !cloudHost.includes('example.com');

  // Treat placeholder URLs as no avatar. Default is a static app asset, not a bucket upload —
  // always serve from the local /img path regardless of cloud config.
  // Also catch previously-stored cloud default URLs (avatar-default.png in the bucket was never
  // uploaded there; any stored URL ending in /avatar-default.png is a broken default).
  if (!avatar || avatar.includes('example.com') || /\/avatar-default\.png$/.test(avatar)) {
