var mongoose    = require('mongoose'),
    ...
    schema = {
      ...
      users        : [{
        ...
        // Denormalized like displayName: the dashboards sort these roster
        // entries, not user documents, so a surname stored on the user is
        // invisible to them unless it is copied across. Absent for users we
        // have no parts for — the client falls back to deriving a surname
        // from displayName, so no backfill is needed.
    ...
    if (/^course:/.test(role.context)) {
      ...
      update = {
        "$set" : {
          ...
          "users.$.givenName"   : user.givenName  || null,
          "users.$.familyName"  : user.familyName || null,
