...
// Matches relative /api/files/ image/asset URLs exported from trinket.io
var TRINKET_FILES_RE = /(\]\(|src=['"])(\/api\/files\/)/g;
...
function createMaterialFromFile(lesson, matFile, user) {
  ...
  var content = (matFile.content || '').replace(TRINKET_FILES_RE, function(_, prefix, path) {
    return prefix + TRINKET_FILES_SOURCE + path;
  });
