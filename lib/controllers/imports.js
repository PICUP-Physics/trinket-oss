...
function importOneTrinket(zip, entry, userId, replace, results) {
  ...
  return Trinket.findByLegacyShortCode(legacyShortCode).exec()
...
function patchUnresolvedRefs(shortCodes, legacyMap) {
  ...
  return chunks.reduce(function(chain, chunk) {
    return chain.then(function(acc) {
      return Material.findByUnresolvedLegacyRefs(chunk).exec()
        .then(function(found) {
...
function importCourse(request, reply) {
  ...
  return readUploadedFile(request.payload.file)
    ...
    .then(function(result) {
      ...
      return createCourseFromChapters(chapters, courseName, user, courseZip, legacyToTrinket)
...
function resolveAllRefs(chapters) {
  ...
  return chunks.reduce(function(chain, chunk) {
    return chain.then(function(acc) {
      return Trinket.findByLegacyShortCodes(chunk).exec()
...
function createCourseFromChapters(chapters, courseName, user, zip, legacyToTrinket) {
    ...
    .then(function(savedCourse) {
      ...
      return chapters.reduce(function(chain, chapter) {
        return chain.then(function(c) {
          return createLessonFromChapter(c, chapter, user, zip, legacyToTrinket);
        });
      }, Promise.resolve(savedCourse));
    });
}

function createLessonFromChapter(course, chapter, user, zip, legacyToTrinket) {
  ...
  return lesson.save()
    .then(function(savedLesson) {
      ...
      return chapter.materials.reduce(function(chain, mat) {
        return chain.then(function() {
          return createMaterialFromFile(savedLesson, mat, user, zip, legacyToTrinket);
        });
      }, Promise.resolve())
      .then(function() {
...
function rehostImportedAssets(content, user, zip) {
...
function createMaterialFromFile(lesson, matFile, user, zip, legacyToTrinket) {
  var name = matFile.materialName || matFile.filename.replace(/\.md$/i, '').replace(/^\d+-/, '').replace(/[-_]/g, ' ');
  var type = matFile.type || 'page';

  return rehostImportedAssets(matFile.content, user, zip).then(function(content) {
