...
function importCourse(request, reply) {
  ...
  return readUploadedFile(request.payload.file)
    ...
    .then(function(result) {
      ...
      return createCourseFromChapters(chapters, courseName, user, courseZip)
      ...
      manifest.lessons.forEach(function(lessonMeta, lessonIdx) {
        ...
        (lessonMeta.materials || []).forEach(function(matMeta, matIdx) {
          ...
          promises.push(entry.async('string').then(function(content) {
            chapter.materials.push({ filename: filename, materialName: matMeta.name, content: content });
          }));
        });
      });
...
function createCourseFromChapters(chapters, courseName, user, zip) {
    ...
    .then(function(savedCourse) {
      ...
      return chapters.reduce(function(chain, chapter) {
        return chain.then(function(c) {
          return createLessonFromChapter(c, chapter, user, zip);
        });
      }, Promise.resolve(savedCourse));
    });
}

function createLessonFromChapter(course, chapter, user, zip) {
  ...
  return lesson.save()
    .then(function(savedLesson) {
      ...
      return chapter.materials.reduce(function(chain, mat) {
        return chain.then(function() {
          return createMaterialFromFile(savedLesson, mat, user, zip);
        });
      }, Promise.resolve())
      .then(function() {
...
function createMaterialFromFile(lesson, matFile, user, zip) {
