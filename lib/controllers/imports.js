...
function patchUnresolvedRefs(shortCodes, legacyMap) {
  ...
  // Firestore $in is capped at 30 values — chunk the query
  var CHUNK = 30;
  var chunks = [];
...
function parseCourseZip(zip) {
  // Collect chapters: { folderName, materials: [{filename, content}] }
  // Handles both flat zips (folder/file.md) and zips with an outer directory
  // (outer/folder/file.md — the outer directory is stripped automatically).
  var chapterMap = {};
  var promises   = [];
  ...
  zip.forEach(function(relativePath, zipEntry) {
    ...
    if (parts.length === 2 && /\.md$/i.test(parts[1])) {
      ...
      // outer-dir/folder/file.md — strip outer dir
      folderName = parts[1];
      filename   = parts[2];
    } else {
      return;
...
function resolveAllRefs(chapters) {
  ...
  // Look up shortCodes in DB — chunk to respect Firestore's 30-value $in limit
  var CHUNK = 30;
  var chunks = [];
