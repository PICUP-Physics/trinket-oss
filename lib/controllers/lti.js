        ...
        .then(function(claims) {
          ...
          return ltiNonceStore.checkAndRecord(claims.nonce, NONCE_TTL_SECONDS).then(function(fresh) {
            ...
            if (messageType === 'LtiDeepLinkingRequest') {
              ...
              // Picker mode detection. Two LMS families behave differently here:
              //  - Canvas exposes TWO placements and OMITS accept_lineitem. We keep using
              //    accept_multiple: false (assignment_selection) -> assignment view; otherwise
              //    (link_selection, accept_multiple:true) -> content view. (Locked; unchanged.)
              //  - Moodle/D2L offer a SINGLE generic "Select content" entry and DO send
              //    accept_lineitem. There we can't lock to one view, so we offer BOTH modes as
              //    tabs; accept_lineitem tells us whether a graded assignment is even allowed.
              var hasLineitem = (dlSettings.accept_lineitem !== undefined);
              var dlMode, assignmentAllowed;
              if (hasLineitem) {
                dlMode = 'both';
                assignmentAllowed = (dlSettings.accept_lineitem === true);
              } else {
                dlMode = (dlSettings.accept_multiple === false) ? 'assignment' : 'content';
                assignmentAllowed = (dlMode === 'assignment');
              }
              // TEMP diagnostic (KEEP until verified on a live Moodle/D2L launch): dump exactly
              // what the platform sent in deep_linking_settings plus our computed mode.
                ...
                return ltiProvision.provisionUser(claims, platform, { isInstructor: isInstructor }).then(function (user) {
                  return ltiTarget.resolveTarget(claims, platform).then(function (target) {
                    ...
                    if (target.assignment && target.assignment.lessonSlug && target.assignment.materialSlug) {
                      ...
                      // Topic landing: the lesson page within the course (no specific material).
                      redirectPath = '/' + target.course.ownerSlug + '/courses/' + target.course.slug +
