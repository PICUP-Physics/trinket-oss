...
module.exports = {
                ...
                .then(function (isInstructor0) {
                  return ltiProvision.provisionUser(claims, platform, { isInstructor: isInstructor0 }).then(function (user) {
                    ...
                    request.yar.set('ltiDeepLink', {
                      ...
                      // accept_line_item distinguishes the placement: assignment_selection sets it
                      // (gradeable) → show assignments; link_selection does not → show course/topics.
                      acceptLineItem: !!dlSettings.accept_line_item,
  ...
  deepLinkPicker: function(request, reply) {
    ...
    // Context-aware picker: assignment_selection (accept_line_item) offers assignments; link_selection
    // offers the whole course + its topics (lessons). Only what makes sense for the placement is shown.
    var contentMode = !dl.acceptLineItem;
    return new Promise(function(resolve, reject) {
      ...
      return request.success({ courses: view, contentMode: contentMode, anyAssignments: anyAssignments, returnConfigured: true });
    }).catch(function (e) {
