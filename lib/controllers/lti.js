      ...
      ltiVerify.verifyLaunchToken(idToken, platform)
        .then(function(claims) {
          ...
          return ltiNonceStore.checkAndRecord(claims.nonce, NONCE_TTL_SECONDS).then(function(fresh) {
            ...
            // mapped role, then establish the trinket session (mirroring POST /api/auth/session:
            // yar.reset + _logIn, callback not awaited, same as auth.js) and land on the course.
            return ltiProvision.provisionUser(claims, platform).then(function(user) {
              return ltiTarget.resolveTarget(claims, platform).then(function(target) {
                var redirectPath = '/welcome';
                var enrollP = Promise.resolve();
                if (target.course) {
                  var role = ltiRoles.mapCourseRole(claims[LTI + 'roles']);
                  enrollP = Promise.resolve(target.course.addUser(user, [role]));  // idempotent
