...
module.exports = {
  ...
  login : async function(request, reply) {
    // Rate limit by IP (20 attempts / 15 min) and by target account (10 attempts / 15 min).
    var ip = request.info.remoteAddress;
    var ipKey      = 'rate:login:ip:'  + ip;
    var targetKey  = 'rate:login:acct:' + request.payload.email;
    var ipCount    = parseInt(await Store.get(ipKey)     || '0', 10) + 1;
    var acctCount  = parseInt(await Store.get(targetKey) || '0', 10) + 1;
    await Store.set(ipKey,     String(ipCount));
    await Store.set(targetKey, String(acctCount));
    if (ipCount   === 1) await Store.expire(ipKey,     900);
    if (acctCount === 1) await Store.expire(targetKey, 900);
    if (ipCount > 20 || acctCount > 10) {
  ...
  downloadExport : function(request, reply) {
    ...
    Export.findById(exportId, function(err, exportRecord) {
      ...
      // Local/dev stacks reach S3 (MinIO) at an internal hostname (e.g.
      // minio:9000) that the browser can't resolve. When S3_PUBLIC_HOST is set,
      // swap the URL's host so the redirect works from the browser. SigV2 query
      // URLs don't sign the host, so the signature stays valid. No-op in prod.
      if (process.env.S3_PUBLIC_HOST) {
