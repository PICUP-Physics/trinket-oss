...
// route when features.requireAuthToFork is true. Picup has no approval tier, so any
// logged-in user passes; anonymous requests (request.user undefined) get a clean 401
// that the embed fork() .fail() handler turns into a "Log in to save…" bar.
// (gcr's isApproved additionally checks user.approved / admin — that tier lives there.)
